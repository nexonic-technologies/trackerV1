...
import {
  Lock, Mail, Eye, EyeOff, Shield, ChevronRight,
  Moon, Sun, BarChart2, FolderKanban, TicketCheck, Users,
} from "lucide-react";
...
const FEATURES = [
  { icon: Users, title: "HR Operations", desc: "People, payroll & attendance", iconBg: "bg-[#ede9fe]", iconColor: "text-[#7c3aed]" },
  { icon: FolderKanban, title: "Projects", desc: "Plan, track & ship on time", iconBg: "bg-[#e0f2fe]", iconColor: "text-[#0ea5e9]" },
  { icon: TicketCheck, title: "Tickets", desc: "Resolve issues faster", iconBg: "bg-[#ffe4ec]", iconColor: "text-[#e11d48]" },
  { icon: BarChart2, title: "Analytics", desc: "Data that drives decisions", iconBg: "bg-[#d1fae5]", iconColor: "text-[#059669]" },
];
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (remembered) { setWorkEmail(remembered); setRememberMe(true); }
  }, []);

  /* ── Login (unchanged business logic) ──────────────────────────────── */
  const handleLogin = async () => {
    if (!workEmail || !password) { toast.error("Please enter both email and password"); return; }
    setLoading(true);
    try {
      ...
      if (response.status === 200 && response.data.accessToken) {
        ...
        try {
          ...
          if (fcmToken) await axiosInstance.post("/auth/store-push-token", { sessionId: response.data.sessionId, fcmToken });
        } catch (fcmError) { console.error("FCM Token registration failed:", fcmError); }
  ...
  return (
    <div className="lmx-login-bg relative overflow-x-hidden flex min-h-screen">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0" aria-hidden>
        ...
        <div style={{ position: "absolute", top: "-10%", right: "-10%", width: "600px", height: "600px", borderRadius: "50%", background: "rgba(155, 143, 212, 0.4)", filter: "blur(90px)" }} />
        <div style={{ position: "absolute", bottom: "-10%", left: "-10%", width: "650px", height: "650px", borderRadius: "50%", background: "rgba(220, 170, 200, 0.35)", filter: "blur(90px)" }} />
        <div style={{ position: "absolute", top: "35%", right: "15%", width: "500px", height: "500px", borderRadius: "50%", background: "rgba(240, 210, 230, 0.3)", filter: "blur(80px)" }} />
      ...
      <button
        type="button"
        onClick={toggleTheme}
        className="absolute top-5 right-5 z-20 p-2 rounded-xl bg-white/60 dark:bg-white/10 backdrop-blur border border-white/40 dark:border-white/10 text-[var(--tracker-ink)] hover:scale-105 active:scale-95 transition-transform"
      ...
      {/* ── Two cards — centered group with generous gap side by side ── */}
      <div className="relative z-10 w-full flex items-center justify-center min-h-screen px-6 py-10">
        <div className="flex items-center justify-between gap-12 lg:gap-32 w-full max-w-[1080px]">
          ...
          <div className="lmx-login-feature-card hidden lg:flex flex-col w-[440px] flex-shrink-0 p-8 gap-5">
            ...
            <div className="flex flex-col gap-2.5">
              {FEATURES.map((feat) => (
                <div key={feat.title} className="lmx-login-inner-row flex items-center gap-3 px-4 py-3">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 ${feat.iconBg}`}>
                    <feat.icon className={`h-5 w-5 ${feat.iconColor}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[var(--tracker-ink)]">{feat.title}</p>
                    <p className="text-xs text-[var(--tracker-ink-subtle)] mt-0.5">{feat.desc}</p>
                  </div>
            ...
            <div className="border-t border-[var(--tracker-border)] pt-4 flex items-center gap-2">
              ...
              <div className="flex-1">
                <p className="text-xs font-medium text-[var(--tracker-ink-muted)]">Secure enterprise access</p>
                <p className="text-[11px] text-[var(--tracker-ink-subtle)]">Your data is safe with us</p>
              </div>
          ...
          <div className="lmx-login-card w-full lg:w-[450px] flex-shrink-0 p-8 sm:p-10 flex flex-col gap-5">
            ...
            {/* Logo inside card (shows on mobile viewport only) */}
            <div className="flex lg:hidden flex-col items-center gap-2 mb-2 select-none">
            ...
            <div className="lg:hidden">
              <p className="lmx-login-eyebrow mb-1">Account Sign-In</p>
              <h2 className="text-2xl font-bold text-[var(--tracker-ink)] tracking-tight">Sign in</h2>
              <p className="text-sm text-[var(--tracker-ink-muted)] mt-1">
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* Work email */}
              <div>
                <label htmlFor="login-email" className="block text-sm font-medium text-[var(--tracker-ink)] mb-1.5">
                  Work email
                </label>
              ...
              {/* Password */}
              <div>
                <label htmlFor="login-password" className="block text-sm font-medium text-[var(--tracker-ink)] mb-1.5">
                  Password
                </label>
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-sm text-[var(--tracker-ink-muted)]">Remember me</span>
                </label>
                <Link
                  to="/forgot-password"
              ...
              <button
                ...
                style={{ background: "linear-gradient(135deg, #3b5bdb 0%, #6c3de8 100%)" }}
              >
                {loading ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" className="lmx-login-social-btn" aria-label="Sign in with Microsoft" onClick={() => toast("Microsoft SSO coming soon")}>
                <MicrosoftIcon />
                <span className="text-sm font-medium text-[var(--tracker-ink)]">Microsoft</span>
              </button>
              <button type="button" className="lmx-login-social-btn" aria-label="Sign in with Google" onClick={() => toast("Google SSO coming soon")}>
                <GoogleIcon />
                <span className="text-sm font-medium text-[var(--tracker-ink)]">Google</span>
              </button>
            </div>
