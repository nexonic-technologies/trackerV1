...
const WorkflowsIndex = () => {
  ...
  const fetchWorkflows = async () => {
    try {
      const res = await axiosInstance.post("/populate/read/workflows", { limit: 500 });
  ...
  const filtered = query.trim()
    ? workflows.filter((w) =>
      [w.name, w.modelName, MODEL_LABELS[w.modelName]?.label, w.triggerType].some(
            ...
            <table className="w-full text-sm" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "var(--tracker-surface-1,#f9fafb)", borderBottom: "1px solid var(--tracker-border,#e5e7eb)" }}>
                  {["Name", "Context", "Module", "Scope", "Steps", "Status", ""].map((h) => (
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="py-20 text-center">
                    ...
                    <p className="text-xs mt-1" style={{ color: "var(--tracker-ink-muted)" }}>
                      {!query && "Add your first unified workflow."}
                    </p>
                  </td></tr>
                ) : (
                  filtered.map((wf) => {
                    const modelMeta = MODEL_LABELS[wf.modelName] || { label: wf.modelName, color: "bg-gray-100 text-gray-650" };
                    const triggerMeta = TRIGGER_LABELS[wf.triggerType] || { label: wf.triggerType, color: "bg-gray-100 text-gray-600" };
                    return (
                      <tr key={wf._id} className="group cursor-pointer transition-colors"
                        ...
                        {/* Name */}
                        <td className="px-5 py-3.5 font-bold" style={{ color: "var(--tracker-ink)" }}>
                          {wf.name}
                        </td>

                        {/* Context / Trigger Type */}
                        <td className="px-5 py-3.5">
                          <span className={`px-2.5 py-0.5 text-xs font-bold rounded-full ${triggerMeta.color}`}>
                            {triggerMeta.label}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                          <DepartmentChip departmentId={wf.conditions?.departmentId || wf.departmentId} />
                        </td>
