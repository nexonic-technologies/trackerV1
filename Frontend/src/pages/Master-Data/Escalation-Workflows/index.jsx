...
import { escalationWorkflowsConfig } from "./config";
...
const MODEL_LABELS = {
  ...
  wfhrequests: { label: "WFH Requests", color: "bg-cyan-100   text-cyan-700" },
  compoffrequests: { label: "CompOff Requests", color: "bg-teal-100   text-teal-700" },
...
const PRIORITY_CHIP = {
  ...
  "Weekly Priority": "bg-purple-100 text-purple-700",
};

const ESCALATE_ICONS = {
  "Reporting Manager": "👤",
  "Department Manager": "🏢",
  "HR": "🛡️",
  "Specific Role": "🎭",
    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      {steps.map((step, i) => (
        <div key={i} className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium"
          ...
          <span className="font-bold">L{step.level}</span>
          <span className="opacity-70">·</span>
          <span>{ESCALATE_ICONS[step.escalateToType] || "→"} {step.escalateToType}</span>
...
const ConditionChips = ({ conditions = {} }) => {
  const chips = [];
  if (conditions.priorityLevel) chips.push({ label: conditions.priorityLevel, cls: PRIORITY_CHIP[conditions.priorityLevel] || "bg-gray-100 text-gray-600" });
  if (conditions.departmentId?.title || conditions.departmentId) chips.push({ label: conditions.departmentId?.name || "Dept filter", cls: "bg-slate-100 text-slate-700" });
  if (!chips.length) return <span className="text-xs" style={{ color: "var(--tracker-ink-muted)" }}>All</span>;
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      {chips.map((c, i) => <span key={i} className={`px-2 py-0.5 text-xs font-medium rounded-full ${c.cls}`}>{c.label}</span>)}
    </div>
  );
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-end"
      ...
      <div className="relative h-full w-full max-w-lg flex flex-col shadow-2xl"
        ...
        <div className="px-6 py-4 flex items-start justify-between gap-4"
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--module-accent,#6366f1)" }}>Escalation Workflow</p>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          {/* Conditions */}
          <div>
            <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--tracker-ink-muted)" }}>Trigger Conditions</p>
            <div className="rounded-xl p-4 space-y-2" style={{ background: "var(--tracker-surface-1,#f9fafb)", border: "1px solid var(--tracker-border,#e5e7eb)" }}>
              {workflow.conditions?.priorityLevel && (
                <div className="flex items-center justify-between text-sm">
                  <span style={{ color: "var(--tracker-ink-muted)" }}>Priority</span>
                  <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full ${PRIORITY_CHIP[workflow.conditions.priorityLevel] || "bg-gray-100 text-gray-600"}`}>
                    {workflow.conditions.priorityLevel}
                  </span>
                </div>
              )}
              {(workflow.conditions?.departmentId?.title || workflow.conditions?.departmentId) && (
                <div className="flex items-center justify-between text-sm">
                  <span style={{ color: "var(--tracker-ink-muted)" }}>Department</span>
                  <span className="font-medium text-xs" style={{ color: "var(--tracker-ink)" }}>
                    {workflow.conditions.departmentId?.title || workflow.conditions.departmentId}
                  </span>
                </div>
              )}
              {!workflow.conditions?.priorityLevel && !workflow.conditions?.departmentId && (
                <p className="text-xs" style={{ color: "var(--tracker-ink-muted)" }}>No conditions — applies to all records</p>
              )}
            </div>
          </div>
          ...
          {/* Steps timeline */}
          <div>
            <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--tracker-ink-muted)" }}>Escalation Steps</p>
                        ...
                        <div className="flex-1 rounded-xl p-4" style={{ background: "var(--tracker-surface-1,#f9fafb)", border: `1px solid ${color}35` }}>
                          <div className="flex items-center justify-between flex-wrap gap-2">
                            <div className="flex items-center gap-2">
                              <span className="text-base">{ESCALATE_ICONS[step.escalateToType] || "→"}</span>
                              <span className="font-semibold text-sm" style={{ color: "var(--tracker-ink)" }}>{step.escalateToType}</span>
                            </div>
                          ...
                          {step.actions?.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 mt-2.5">
                              {step.actions.map((a, ai) => (
                                <span key={ai} className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-600">{a}</span>
                              ))}
                            </div>
                          )}
                          {step.updateStatusTo && (
                            <p className="text-xs mt-2" style={{ color: "var(--tracker-ink-muted)" }}>→ Status: <strong>{step.updateStatusTo}</strong></p>
                          )}
                        </div>
                      </div>
                    );
...
const EscalationWorkflows = () => {
  ...
  const basePath = escalationWorkflowsConfig.basePath;

  const fetchWorkflows = async () => {
    try {
      const res = await axiosInstance.post("/populate/read/escalationworkflows", { limit: 500 });
  ...
  const handleDelete = async (workflow) => {
    ...
    try {
      await axiosInstance.delete(`/populate/delete/escalationworkflows/${workflow._id}`);
      fetchWorkflows();
  ...
  const filtered = query.trim()
    ? workflows.filter((w) =>
      [w.name, w.modelName, MODEL_LABELS[w.modelName]?.label].some(
    ...
    <div className="min-h-full" style={{ background: "var(--tracker-canvas-muted,#f5f6fa)" }}>
      ...
      <div className="px-6 pt-6 pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold mt-0.5" style={{ color: "var(--tracker-ink)" }}>Escalation Workflows</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--tracker-ink-muted)" }}>Manage escalation rules across modules</p>
        </div>
            ...
            <table className="w-full text-sm" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "var(--tracker-surface-1,#f9fafb)", borderBottom: "1px solid var(--tracker-border,#e5e7eb)" }}>
                  {["Workflow", "Module", "Conditions", "Steps", "Status", ""].map((h) => (
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="py-20 text-center">
                    ...
                    <p className="text-xs mt-1" style={{ color: "var(--tracker-ink-muted)" }}>
                      {!query && "Add your first escalation workflow."}
                    </p>
                  </td></tr>
                ) : (
                  filtered.map((wf) => {
                    ...
                    return (
                      <tr key={wf._id} className="group cursor-pointer transition-colors"
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <span className="w-8 h-8 rounded-lg flex items-center justify-center text-sm shrink-0"
                              style={{ background: "var(--module-accent,#6366f1)18", color: "var(--module-accent,#6366f1)" }}>
                              ⚡
                            </span>
                            <span className="font-semibold" style={{ color: "var(--tracker-ink)" }}>{wf.name}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                          <ConditionChips conditions={wf.conditions} />
                        </td>
