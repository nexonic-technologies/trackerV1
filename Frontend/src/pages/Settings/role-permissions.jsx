...
const DataAccessTab = ({ models, policies, navMap, navChanges, sidebars, onToggle, onModelToggleAll, onGlobalToggle, onNavToggle, onSave, saving, message }) => {
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 gap-4">
            ...
            {/* ── Section 2: Navigation Menu Tree (shown below table) ── */}
            <div className="flex flex-col gap-3">
                <div className="border-t border-gray-100 dark:border-gray-800/60 pt-4 mt-2">
                    <h3 className="text-base font-black text-gray-850 dark:text-gray-100 tracking-tight">Navigation Menu Structure</h3>
                    <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mt-0.5">
                        Set visibility for the entire menu list below. When the role gets access permission above, the menu item resolves automatically.
                    </p>
                </div>

                <PremiumCard noPad className="overflow-hidden max-h-[350px] flex flex-col">
                    <div className="overflow-auto flex-1">
                        {parents.length === 0 ? (
                            <div className="p-8 text-center text-gray-400 dark:text-gray-600 text-sm">No sidebar menu items found</div>
                        ) : (
                            parents.map(parent => {
                                const parentIdStr = getCleanId(parent._id);
                                const parentVisibility = navChanges[parentIdStr] ?? parent.visibility;
                                const isPublic = parentVisibility === 'public';
                                const isChanged = navChanges[parentIdStr] !== undefined;

                                const parentChildren = children.filter(
                                    c => getCleanId(c.parentId) === parentIdStr
                                );
                                ...
                                return (
                                    <div key={parent._id}>
                                        ...
                                        <div
                                            className={`
                                                flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-700/50
                                                transition-all duration-200
                                                ${isPublic
                                                    ? 'bg-emerald-50/60 dark:bg-emerald-950/15'
                                                    : 'bg-white dark:bg-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-700/20'
                                                }
                                            `}
                                        >
                                            ...
                                            <div className="flex-1 flex items-center gap-2 min-w-0">
                                                ...
                                                {parent._resolvedModel && (
                                                    <span className="flex-shrink-0 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded-md border border-blue-200/60 dark:border-blue-800/40">
                                                        {parent._resolvedModel}
                                                    </span>
                                                )}
                                            </div>
                                            <code className="text-xs text-gray-400 dark:text-gray-500 font-mono mr-3 hidden sm:block">
                                                {parent.mainRoute}
                                            </code>
                                            <button
                                                onClick={() => onNavToggle(parent._id)}
                                                className={`
                                                    relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold
                                                    border-2 transition-all duration-300
                                                    ${isPublic
                                                        ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700 shadow-sm shadow-emerald-200/50'
                                                        : 'bg-gray-100 dark:bg-gray-700/60 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-700'
                                                    }
                                                `}
                                            >
                                                {isPublic
                                                    ? <><EyeIcon className="w-3.5 h-3.5" /> Public</>
                                                    : <><EyeSlashIcon className="w-3.5 h-3.5" /> Protected</>
                                                }
                                                {isChanged && (
                                                    <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-amber-400 border border-white dark:border-gray-800" />
                                                )}
                                        ...
                                        {parentChildren.map(child => {
                                            const childIdStr = getCleanId(child._id);
                                            const childVisibility = navChanges[childIdStr] ?? child.visibility;
                                            const childPublic = childVisibility === 'public';
                                            const childChanged = navChanges[childIdStr] !== undefined;

                                            return (
                                                <div
                                                    key={child._id}
                                                    className={`
                                                        flex items-center gap-3 pl-10 pr-4 py-2.5
                                                        border-b border-gray-50 dark:border-gray-700/30 last:border-0
                                                        transition-all duration-200
                                                        ${childPublic
                                                            ? 'bg-emerald-50/40 dark:bg-emerald-950/10'
                                                            : 'bg-white dark:bg-gray-800/30 hover:bg-gray-50/60 dark:hover:bg-gray-700/10'
                                                        }
                                                    `}
                                                >
                                                    ...
                                                    <button
                                                        onClick={() => onNavToggle(child._id)}
                                                        className={`
                                                            relative flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold
                                                            border-2 transition-all duration-300
                                                            ${childPublic
                                                                ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700 shadow-sm shadow-emerald-200/50'
                                                                : 'bg-gray-100 dark:bg-gray-700/60 text-gray-400 dark:text-gray-500 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-700'
                                                            }
                                                        `}
                                                    >
                                                        {childPublic
                                                            ? <><EyeIcon className="w-3 h-3" /> Public</>
                                                            : <><EyeSlashIcon className="w-3 h-3" /> Protected</>
                                                        }
                                                        {childChanged && (
                                                            <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-amber-400 border border-white dark:border-gray-800" />
                                                        )}
            ...
            {/* Nav legend */}
            {Object.keys(navChanges).length > 0 && (
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-amber-50/80 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-800/40">
                    <span className="w-2 h-2 rounded-full bg-amber-400 flex-shrink-0 animate-pulse" />
                    <p className="text-xs font-semibold text-amber-700 dark:text-amber-300">
                        {Object.keys(navChanges).length} nav menu item{Object.keys(navChanges).length > 1 ? 's' : ''} modified. Staged modifications are marked with a yellow dot indicator and will be saved when you click <strong>Save Changes</strong> below.
                    </p>
                </div>
            )}
            {Object.keys(navChanges).length === 0 && (
                <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-600">
                    <span className="flex items-center gap-1 font-bold"><EyeIcon className="w-3 h-3 text-emerald-500" /> Public (Always)</span>
                    <span className="text-gray-300 dark:text-gray-700">= always shown to everyone</span>
                    <span className="mx-1">·</span>
                    <span className="flex items-center gap-1 font-bold"><EyeSlashIcon className="w-3 h-3 text-gray-400" /> Protected (Auto)</span>
                    <span className="text-gray-300 dark:text-gray-700">= shown only if Read permission is granted above</span>
                </div>
            )}
